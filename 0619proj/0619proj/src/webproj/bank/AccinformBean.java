...
public class AccinformBean {
	...
	public int customerBank(AccountInform accinform) {
		connect();
		int csBankId = 0;
		String sql = "SELECT mem_bank FROM customer_info WHERE mem_accid=?";
		ResultSet rs = null;
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, accinform.getMem_accid());
			...
			rs = pstmt.executeQuery();
			if(rs.next()) {
				csBankId = rs.getInt("mem_bank");
				System.out.println(csBankId);
			}
		} catch(Exception e) {
			e.printStackTrace();
		} finally {
	...
	public boolean viewAccount(AccountInform accinform) {
		connect();
		String sql = "SELECT mem_context, mem_wireacc, mem_curranc, mem_wirecur FROM account_info WHERE mem_id=?;";
		try {
			pstmt = conn.prepareStatement(sql);
		} catch(Exception e) {
			e.printStackTrace();
